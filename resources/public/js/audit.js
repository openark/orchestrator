    ...
    function displayAudit(auditEntries) {
        ...
        auditEntries.forEach(function (audit) {
            ...
            if (audit.AuditInstanceKey.Hostname) {
                ...
                $('<a/>',  { text: joinHostPort(audit.AuditInstanceKey.Hostname, audit.AuditInstanceKey.Port) , href: uri}).wrap($("<td/>")).parent().appendTo(row);
            } else {
                jQuery('<td/>', { text: joinHostPort(audit.AuditInstanceKey.Hostname, audit.AuditInstanceKey.Port) }).appendTo(row);
            }
