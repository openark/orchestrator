...
function discover(hostname, port) {
    ...
    $.get(appUrl(uri), function (operationResult) {
        ...
        if (operationResult.Code == "ERROR" || operationResult.Details == null) {
            ...
            addInfo('Discovered <a href="' + appUrl('/web/search?s='+encodeURIComponent(joinHostPort(instance.Key.Hostname,instance.Key.Port))) + '" class="alert-link">'
                    +joinHostPort(instance.Key.Hostname,instance.Key.Port)+'</a>'
                );
        }
    }, "json");
