...
func applyEnvironmentVariables(topologyRecovery *TopologyRecovery) []string {
	...
	env = append(env, fmt.Sprintf("ORC_IS_MASTER=%s", fmt.Sprintf("%t", analysisEntry.IsMaster)))
	env = append(env, fmt.Sprintf("ORC_IS_CO_MASTER=%s", fmt.Sprintf("%t", analysisEntry.IsCoMaster)))
