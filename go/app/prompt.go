...
Cheatsheet:
    ...
    Topology refactoring, generic aka "smart" commands
        These operations let orchestrator pick the best course of action for relocating replicas. It may choose to use
        ...
        In case a of a multi-step operation, failure may result in replicas only moving halfway to destination point. Nonetheless
    ...
    Topology refactoring using classic MySQL replication commands
        ...
        These commands require connected topology: replicas that are up and running; a lagging, stopped or
        failed replica will disable use of most these commands. At least one, and typically two or more replicas
        will be stopped for a short time during these operations.

        move-up
            Move a replica one level up the topology; makes it replicate from its grandparent and become sibling of
            its parent. It is OK if the instance's master is not replicating. Examples:

            orchestrator -c move-up -i replica.to.move.up.com:3306
