...
func TestMkInsertOdkuThree(t *testing.T) {
	...
	s3 := `INSERT  INTO database_instance
        ...
        ON DUPLICATE KEY UPDATE
                hostname=VALUES(hostname), port=VALUES(port), last_checked=VALUES(last_checked), last_attempted_check=VALUES(last_attempted_check), last_check_partial_success=VALUES(last_check_partial_success), uptime=VALUES(uptime), server_id=VALUES(server_id), server_uuid=VALUES(server_uuid), version=VALUES(version), major_version=VALUES(major_version), version_comment=VALUES(version_comment), binlog_server=VALUES(binlog_server), read_only=VALUES(read_only), binlog_format=VALUES(binlog_format), binlog_row_image=VALUES(binlog_row_image), log_bin=VALUES(log_bin), log_subordinate_updates=VALUES(log_subordinate_updates), binary_log_file=VALUES(binary_log_file), binary_log_pos=VALUES(binary_log_pos), main_host=VALUES(main_host), main_port=VALUES(main_port), subordinate_sql_running=VALUES(subordinate_sql_running), subordinate_io_running=VALUES(subordinate_io_running), replication_sql_thread_state=VALUES(replication_sql_thread_state), replication_io_thread_state=VALUES(replication_io_thread_state), has_replication_filters=VALUES(has_replication_filters), supports_oracle_gtid=VALUES(supports_oracle_gtid), oracle_gtid=VALUES(oracle_gtid), main_uuid=VALUES(main_uuid), ancestry_uuid=VALUES(ancestry_uuid), executed_gtid_set=VALUES(executed_gtid_set), gtid_mode=VALUES(gtid_mode), gtid_purged=VALUES(gtid_purged), gtid_errant=VALUES(gtid_errant), mariadb_gtid=VALUES(mariadb_gtid), pseudo_gtid=VALUES(pseudo_gtid), main_log_file=VALUES(main_log_file), read_main_log_pos=VALUES(read_main_log_pos), relay_main_log_file=VALUES(relay_main_log_file), exec_main_log_pos=VALUES(exec_main_log_pos), relay_log_file=VALUES(relay_log_file), relay_log_pos=VALUES(relay_log_pos), last_sql_error=VALUES(last_sql_error), last_io_error=VALUES(last_io_error), seconds_behind_main=VALUES(seconds_behind_main), subordinate_lag_seconds=VALUES(subordinate_lag_seconds), sql_delay=VALUES(sql_delay), num_subordinate_hosts=VALUES(num_subordinate_hosts), subordinate_hosts=VALUES(subordinate_hosts), cluster_name=VALUES(cluster_name), suggested_cluster_alias=VALUES(suggested_cluster_alias), data_center=VALUES(data_center), region=VALUES(region),
								physical_environment=VALUES(physical_environment), replication_depth=VALUES(replication_depth), is_co_main=VALUES(is_co_main), replication_credentials_available=VALUES(replication_credentials_available), has_replication_credentials=VALUES(has_replication_credentials), allow_tls=VALUES(allow_tls), semi_sync_enforced=VALUES(semi_sync_enforced), semi_sync_available=VALUES(semi_sync_available),
								semi_sync_main_enabled=VALUES(semi_sync_main_enabled), semi_sync_main_timeout=VALUES(semi_sync_main_timeout), semi_sync_main_wait_for_subordinate_count=VALUES(semi_sync_main_wait_for_subordinate_count), semi_sync_replica_enabled=VALUES(semi_sync_replica_enabled), semi_sync_main_status=VALUES(semi_sync_main_status), semi_sync_main_clients=VALUES(semi_sync_main_clients), semi_sync_replica_status=VALUES(semi_sync_replica_status),
								instance_alias=VALUES(instance_alias), last_discovery_latency=VALUES(last_discovery_latency), replication_group_name=VALUES(replication_group_name), replication_group_is_single_primary_mode=VALUES(replication_group_is_single_primary_mode), replication_group_member_state=VALUES(replication_group_member_state), replication_group_member_role=VALUES(replication_group_member_role), replication_group_members=VALUES(replication_group_members), replication_group_primary_host=VALUES(replication_group_primary_host), replication_group_primary_port=VALUES(replication_group_primary_port), last_seen=VALUES(last_seen)
        `
	a3 := `
