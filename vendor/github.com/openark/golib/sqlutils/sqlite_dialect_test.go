...
func TestToSqlite3AlterTable(t *testing.T) {
	...
	}
	{
		...
		test.S(t).ExpectEquals(result, stripSpaces(`
			create index
				main_host_port_idx_database_instance
				on database_instance (main_host, main_port)
			`))
	}
	{
		statement := `
...
func TestCreateIndex(t *testing.T) {
	{
		statement := `
			create index
				main_host_port_idx_database_instance
				on database_instance (main_host(128), main_port)
		`
		result := stripSpaces(ToSqlite3Dialect(statement))
		test.S(t).ExpectEquals(result, stripSpaces(`
			create index
...
func TestToSqlite3GeneralConversions(t *testing.T) {
	...
	{
		statement := "AND main_instance.last_attempted_check <= main_instance.last_seen + interval ? minute"
		result := ToSqlite3Dialect(statement)
		test.S(t).ExpectEquals(result, "AND main_instance.last_attempted_check <= datetime(main_instance.last_seen, printf('+%d minute', ?))")
	}
	{
		statement := "select concat(main_instance.port, '') as port"
		result := ToSqlite3Dialect(statement)
		test.S(t).ExpectEquals(result, "select (main_instance.port || '') as port")
	}
	{
